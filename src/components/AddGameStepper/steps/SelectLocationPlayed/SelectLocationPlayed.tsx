import { Autocomplete, Grid, TextField, Typography, AutocompleteInputChangeReason } from "@mui/material";
    ...
    return (
        <Grid container spacing={2}>
            ...
            {/* <Grid container spacing={2} item xs={12}>
                ...
                {displayResults.map((location, i) =>
                    <Grid key={`${location.name}-card-${i}`} item xs={3}>
            ...
            {displayResults?.filter(location => location.type === GameTypeEnum.CONTROL).length ? <Grid container spacing={2} item xs={12}>
                <Grid item xs={12}>
                    <Typography variant={"h4"}>
                        {"Control"}
                    </Typography>
                ...
                {displayResults?.filter(location => location.type === GameTypeEnum.CONTROL).map(location =>
                    <Grid key={`${location.name}-card`} item xs={3}>
                        <LocationCard onClick={() => handleCardOnClick(location)} location={location} />
                    </Grid>
                )}
            </Grid> : null}
            {displayResults?.filter(location => location.type === GameTypeEnum.HYBRID).length ? <Grid container spacing={2} item xs={12}>
                <Grid item xs={12}>
                    <Typography variant={"h4"}>
                        {"Hybrid"}
                    </Typography>
            ...
            {displayResults?.filter(location => location.type === GameTypeEnum.ESCORT).length ? <Grid container spacing={2} item xs={12}>
                <Grid item xs={12}>
                    <Typography variant={"h4"}>
                        {"Escort"}
                    </Typography>
            ...
            {displayResults?.filter(location => location.type === GameTypeEnum.PUSH).length ? <Grid container spacing={2} item xs={12}>
                <Grid item xs={12}>
                    <Typography variant={"h4"}>
                        {"Push"}
                    </Typography>
